...
セキュリティ監査の一環として、既存のWebアプリケーションにセキュリティ脆弱性が報告されました。まず問題のアプリケーションを起動し、正常な動作を確認します。

この演習では以下を達成します：
- 脆弱性を含むWebアプリケーションの起動と動作確認
- 正常な検索機能の理解（次の演習での攻撃との対比のため）  
- アプリケーションの基本構造とSQL クエリ実装の把握

動作確認が完了したら、次の演習でSQL Injection攻撃を実際に実行し、この脆弱性がどのような危険をもたらすかを体験します。
